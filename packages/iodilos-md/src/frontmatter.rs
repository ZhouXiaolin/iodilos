//! YAML-style frontmatter (`---\n…\n---`) extraction, ported from leaf.
//!
//! Only flat `key: value` pairs (plus block scalars `>`/`|` and `- item` lists)
//! are supported — enough for the typical metadata block, without a YAML dep.

/// Extract a leading `---`-delimited frontmatter block from `src`.
///
/// Returns `(content_without_frontmatter, Some(pairs))` when the document
/// opens with a closed frontmatter block holding at least one pair; otherwise
/// `(src, None)`.
