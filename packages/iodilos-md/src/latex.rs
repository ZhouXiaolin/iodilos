//! LaTeX → Unicode conversion, ported from leaf's `latex.rs`.
//!
//! [`to_unicode`] turns common LaTeX math commands into their Unicode
//! equivalents (`\frac{a}{b}` → `a/b`, `^{...}` → superscripts, `\sqrt{x}` →
//! `√(x)`, …), backed by the `unicodeit` table plus local post-processing for
//! constructs `unicodeit` does not handle.
...
fn postprocess(input: &str) -> String {
    ...
    while i < input.len() {
        ...
        if input[i..].starts_with("\\binom{")
            && let Some((output, end)) = parse_binom(input, i)
        {
            result.push_str(&output);
            i = end;
            continue;
